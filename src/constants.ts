import { CountryInfo, ChecklistItem } from "./types";

export const COUNTRIES: CountryInfo[] = [
  {
    id: "jp",
    name: "일본",
    nameEn: "Japan",
    emoji: "🇯🇵",
    voltage: "100V",
    plugType: "Type A (2-pin)",
    weatherSummary: "현재 대부분 맑음, 낮 기온 18-22도 예상. 야외 활동하기 좋은 날씨입니다.",
    forecast: [
      { day: "월", temp: "21°", condition: "☀️" },
      { day: "화", temp: "22°", condition: "☀️" },
      { day: "수", temp: "19°", condition: "☁️" },
      { day: "목", temp: "18°", condition: "🌧️" },
      { day: "금", temp: "20°", condition: "⛅" },
      { day: "토", temp: "23°", condition: "☀️" },
      { day: "일", temp: "22°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "¥", code: "JPY", name: "엔", nameEn: "Yen" }
  },
  {
    id: "us",
    name: "미국",
    nameEn: "USA",
    emoji: "🇺🇸",
    voltage: "120V",
    plugType: "Type A / B",
    weatherSummary: "뉴욕 기준 흐림, 기온 15-20도. 일교차가 크니 가벼운 겉옷을 챙기세요.",
    forecast: [
      { day: "월", temp: "18°", condition: "☁️" },
      { day: "화", temp: "17°", condition: "🌧️" },
      { day: "수", temp: "16°", condition: "🌧️" },
      { day: "목", temp: "19°", condition: "⛅" },
      { day: "금", temp: "21°", condition: "☀️" },
      { day: "토", temp: "22°", condition: "☀️" },
      { day: "일", temp: "20°", condition: "☁️" },
    ],
    region: 'North America',
    currency: { symbol: "$", code: "USD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "cn",
    name: "중국",
    nameEn: "China",
    emoji: "🇨🇳",
    voltage: "220V",
    plugType: "Type A / C / I",
    weatherSummary: "베이징 기준 맑음, 기온 20-25도. 대기질 보통. 야외 활동 시 마스크 착용 권장.",
    forecast: [
      { day: "월", temp: "23°", condition: "☀️" },
      { day: "화", temp: "25°", condition: "☀️" },
      { day: "수", temp: "24°", condition: "⛅" },
      { day: "목", temp: "22°", condition: "☁️" },
      { day: "금", temp: "21°", condition: "☀️" },
      { day: "토", temp: "24°", condition: "☀️" },
      { day: "일", temp: "26°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "¥", code: "CNY", name: "위안", nameEn: "Yuan" }
  },
  {
    id: "sg",
    name: "싱가포르",
    nameEn: "Singapore",
    emoji: "🇸🇬",
    voltage: "230V",
    plugType: "Type G (3-pin)",
    weatherSummary: "열대 기후. 스콜(갑작스런 비) 주의, 기온 28-32도 습도 높음. 항상 우산을 휴대하세요.",
    forecast: [
      { day: "월", temp: "31°", condition: "⛈️" },
      { day: "화", temp: "30°", condition: "⛈️" },
      { day: "수", temp: "32°", condition: "🌦️" },
      { day: "목", temp: "31°", condition: "☀️" },
      { day: "금", temp: "32°", condition: "☀️" },
      { day: "토", temp: "33°", condition: "🌦️" },
      { day: "일", temp: "31°", condition: "⛈️" },
    ],
    region: 'Asia',
    currency: { symbol: "$", code: "SGD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "uk",
    name: "영국",
    nameEn: "United Kingdom",
    emoji: "🇬🇧",
    voltage: "230V",
    plugType: "Type G (3-pin)",
    weatherSummary: "런던 기준 흐리고 가끔 비. 기온 12-18도. 변덕스러운 날씨에 대비해 우비나 우산을 준비하세요.",
    forecast: [
      { day: "월", temp: "15°", condition: "☁️" },
      { day: "화", temp: "14°", condition: "🌧️" },
      { day: "수", temp: "16°", condition: "🌦️" },
      { day: "목", temp: "17°", condition: "☁️" },
      { day: "금", temp: "18°", condition: "⛅" },
      { day: "토", temp: "16°", condition: "🌧️" },
      { day: "일", temp: "15°", condition: "☁️" },
    ],
    region: 'Europe',
    currency: { symbol: "£", code: "GBP", name: "파운드", nameEn: "Pound" }
  },
  {
    id: "de",
    name: "독일",
    nameEn: "Germany",
    emoji: "🇩🇪",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "대체로 흐리고 비 가능성 있음. 기온 10-15도. 쌀쌀한 날씨이니 따뜻한 옷차림이 필요합니다.",
    forecast: [
      { day: "월", temp: "12°", condition: "☁️" },
      { day: "화", temp: "11°", condition: "🌧️" },
      { day: "수", temp: "13°", condition: "🌧️" },
      { day: "목", temp: "14°", condition: "⛅" },
      { day: "금", temp: "15°", condition: "☀️" },
      { day: "토", temp: "14°", condition: "☁️" },
      { day: "일", temp: "13°", condition: "🌧️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "fr",
    name: "프랑스",
    nameEn: "France",
    emoji: "🇫🇷",
    voltage: "230V",
    plugType: "Type C / E",
    weatherSummary: "파리 기준 맑음. 기온 15-22도 일교차 주의. 낮에는 따뜻하지만 저녁엔 쌀쌀할 수 있습니다.",
    forecast: [
      { day: "월", temp: "19°", condition: "☀️" },
      { day: "화", temp: "21°", condition: "☀️" },
      { day: "수", temp: "22°", condition: "☀️" },
      { day: "목", temp: "20°", condition: "⛅" },
      { day: "금", temp: "18°", condition: "☁️" },
      { day: "토", temp: "19°", condition: "☀️" },
      { day: "일", temp: "21°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "vn",
    name: "베트남",
    nameEn: "Vietnam",
    emoji: "🇻🇳",
    voltage: "220V",
    plugType: "Type A / C / G",
    weatherSummary: "고온 다습. 하노이 기준 28-32도 습도 높음. 통풍이 잘 되는 옷을 준비하세요.",
    forecast: [
      { day: "월", temp: "30°", condition: "⛅" },
      { day: "화", temp: "31°", condition: "☀️" },
      { day: "수", temp: "32°", condition: "☀️" },
      { day: "목", temp: "30°", condition: "🌦️" },
      { day: "금", temp: "29°", condition: "⛈️" },
      { day: "토", temp: "31°", condition: "☀️" },
      { day: "일", temp: "32°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "₫", code: "VND", name: "동", nameEn: "Dong" }
  },
  {
    id: "th",
    name: "태국",
    nameEn: "Thailand",
    emoji: "🇹🇭",
    voltage: "220V",
    plugType: "Type A / B / C / O",
    weatherSummary: "방콕 기준 무더운 날씨. 기온 30-35도 예상. 자외선이 강하니 선크림과 모자를 챙기세요.",
    forecast: [
      { day: "월", temp: "33°", condition: "☀️" },
      { day: "화", temp: "34°", condition: "☀️" },
      { day: "수", temp: "35°", condition: "☀️" },
      { day: "목", temp: "33°", condition: "⛅" },
      { day: "금", temp: "32°", condition: "🌦️" },
      { day: "토", temp: "34°", condition: "☀️" },
      { day: "일", temp: "35°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "฿", code: "THB", name: "바트", nameEn: "Baht" }
  },
  {
    id: "tw",
    name: "대만",
    nameEn: "Taiwan",
    emoji: "🇹🇼",
    voltage: "110V",
    plugType: "Type A / B",
    weatherSummary: "타이베이 기준 흐림. 기온 22-26도. 실내 에어컨이 강할 수 있으니 가벼운 가디건을 추천합니다.",
    forecast: [
      { day: "월", temp: "24°", condition: "☁️" },
      { day: "화", temp: "25°", condition: "☁️" },
      { day: "수", temp: "23°", condition: "🌧️" },
      { day: "목", temp: "22°", condition: "🌧️" },
      { day: "금", temp: "24°", condition: "⛅" },
      { day: "토", temp: "26°", condition: "☀️" },
      { day: "일", temp: "25°", condition: "⛅" },
    ],
    region: 'Asia',
    currency: { symbol: "NT$", code: "TWD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "hk",
    name: "홍콩",
    nameEn: "Hong Kong",
    emoji: "🇭🇰",
    voltage: "220V",
    plugType: "Type G (3-pin)",
    weatherSummary: "흐리고 습함. 기온 24-28도. 소나기 가능성이 있으니 우산을 챙기세요.",
    forecast: [
      { day: "월", temp: "26°", condition: "☁️" },
      { day: "화", temp: "27°", condition: "🌦️" },
      { day: "수", temp: "28°", condition: "☀️" },
      { day: "목", temp: "26°", condition: "⛈️" },
      { day: "금", temp: "25°", condition: "🌧️" },
      { day: "토", temp: "27°", condition: "⛅" },
      { day: "일", temp: "28°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "$", code: "HKD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "ca",
    name: "캐나다",
    nameEn: "Canada",
    emoji: "🇨🇦",
    voltage: "120V",
    plugType: "Type A / B",
    weatherSummary: "토론토 기준 맑고 쾌적함. 기온 15-22도 예상.",
    forecast: [
      { day: "월", temp: "18°", condition: "☀️" },
      { day: "화", temp: "20°", condition: "☀️" },
      { day: "수", temp: "21°", condition: "⛅" },
      { day: "목", temp: "19°", condition: "☁️" },
      { day: "금", temp: "17°", condition: "🌧️" },
      { day: "토", temp: "19°", condition: "☀️" },
      { day: "일", temp: "21°", condition: "☀️" },
    ],
    region: 'North America',
    currency: { symbol: "$", code: "CAD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "au",
    name: "호주",
    nameEn: "Australia",
    emoji: "🇦🇺",
    voltage: "230V",
    plugType: "Type I",
    weatherSummary: "시드니 기준 화창함. 기온 18-24도. 일교차 주의.",
    forecast: [
      { day: "월", temp: "22°", condition: "☀️" },
      { day: "화", temp: "23°", condition: "☀️" },
      { day: "수", temp: "21°", condition: "⛅" },
      { day: "목", temp: "20°", condition: "☁️" },
      { day: "금", temp: "22°", condition: "☀️" },
      { day: "토", temp: "24°", condition: "☀️" },
      { day: "일", temp: "23°", condition: "☀️" },
    ],
    region: 'Oceania',
    currency: { symbol: "$", code: "AUD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "ph",
    name: "필리핀",
    nameEn: "Philippines",
    emoji: "🇵🇭",
    voltage: "220V",
    plugType: "Type A / B / C",
    weatherSummary: "마닐라 기준 무덥고 습함. 기온 28-33도. 소나기 잦음.",
    forecast: [
      { day: "월", temp: "31°", condition: "⛈️" },
      { day: "화", temp: "32°", condition: "🌦️" },
      { day: "수", temp: "33°", condition: "☀️" },
      { day: "목", temp: "31°", condition: "⛈️" },
      { day: "금", temp: "30°", condition: "🌧️" },
      { day: "토", temp: "32°", condition: "☀️" },
      { day: "일", temp: "33°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "₱", code: "PHP", name: "페소", nameEn: "Peso" }
  },
  {
    id: "id",
    name: "인도네시아",
    nameEn: "Indonesia",
    emoji: "🇮🇩",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "자카르타 기준 고온다습. 기온 27-32도.",
    forecast: [
      { day: "월", temp: "30°", condition: "🌦️" },
      { day: "화", temp: "31°", condition: "☀️" },
      { day: "수", temp: "32°", condition: "☀️" },
      { day: "목", temp: "30°", condition: "⛈️" },
      { day: "금", temp: "29°", condition: "🌧️" },
      { day: "토", temp: "31°", condition: "☀️" },
      { day: "일", temp: "32°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "Rp", code: "IDR", name: "루피아", nameEn: "Rupiah" }
  },
  {
    id: "it",
    name: "이탈리아",
    nameEn: "Italy",
    emoji: "🇮🇹",
    voltage: "230V",
    plugType: "Type C / F / L",
    weatherSummary: "로마 기준 맑고 따뜻함. 기온 18-26도.",
    forecast: [
      { day: "월", temp: "22°", condition: "☀️" },
      { day: "화", temp: "24°", condition: "☀️" },
      { day: "수", temp: "25°", condition: "☀️" },
      { day: "목", temp: "23°", condition: "⛅" },
      { day: "금", temp: "21°", condition: "☁️" },
      { day: "토", temp: "23°", condition: "☀️" },
      { day: "일", temp: "24°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "es",
    name: "스페인",
    nameEn: "Spain",
    emoji: "🇪🇸",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "마드리드 기준 화창함. 기온 20-28도. 건조함.",
    forecast: [
      { day: "월", temp: "25°", condition: "☀️" },
      { day: "화", temp: "27°", condition: "☀️" },
      { day: "수", temp: "28°", condition: "☀️" },
      { day: "목", temp: "26°", condition: "⛅" },
      { day: "금", temp: "24°", condition: "☀️" },
      { day: "토", temp: "26°", condition: "☀️" },
      { day: "일", temp: "27°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "my",
    name: "말레이시아",
    nameEn: "Malaysia",
    emoji: "🇲🇾",
    voltage: "230V",
    plugType: "Type G (3-pin)",
    weatherSummary: "쿠알라룸푸르 기준 덥고 습함. 기온 27-33도.",
    forecast: [
      { day: "월", temp: "31°", condition: "⛈️" },
      { day: "화", temp: "32°", condition: "🌦️" },
      { day: "수", temp: "33°", condition: "☀️" },
      { day: "목", temp: "32°", condition: "⛈️" },
      { day: "금", temp: "31°", condition: "🌧️" },
      { day: "토", temp: "32°", condition: "☀️" },
      { day: "일", temp: "33°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "RM", code: "MYR", name: "링깃", nameEn: "Ringgit" }
  },
  {
    id: "mx",
    name: "멕시코",
    nameEn: "Mexico",
    emoji: "🇲🇽",
    voltage: "127V",
    plugType: "Type A / B",
    weatherSummary: "멕시코시티 기준 맑음. 기온 15-26도. 일교차 및 고산지대 주의.",
    forecast: [
      { day: "월", temp: "22°", condition: "☀️" },
      { day: "화", temp: "24°", condition: "☀️" },
      { day: "수", temp: "25°", condition: "☀️" },
      { day: "목", temp: "23°", condition: "🌤️" },
      { day: "금", temp: "21°", condition: "⛈️" },
      { day: "토", temp: "23°", condition: "☀️" },
      { day: "일", temp: "24°", condition: "☀️" },
    ],
    region: 'North America',
    currency: { symbol: "$", code: "MXN", name: "페소", nameEn: "Peso" }
  },
  {
    id: "ch",
    name: "스위스",
    nameEn: "Switzerland",
    emoji: "🇨🇭",
    voltage: "230V",
    plugType: "Type J",
    weatherSummary: "취리히 기준 구름 조금. 기온 8-18도. 산악지대 추위 대비 필수.",
    forecast: [
      { day: "월", temp: "14°", condition: "🌤️" },
      { day: "화", temp: "15°", condition: "☀️" },
      { day: "수", temp: "12°", condition: "☁️" },
      { day: "목", temp: "10°", condition: "🌧️" },
      { day: "금", temp: "13°", condition: "🌤️" },
      { day: "토", temp: "16°", condition: "☀️" },
      { day: "일", temp: "17°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "Fr.", code: "CHF", name: "프랑", nameEn: "Franc" }
  },
  {
    id: "nl",
    name: "네덜란드",
    nameEn: "Netherlands",
    emoji: "🇳🇱",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "암스테르담 기준 바람이 강하고 흐림. 기온 10-16도.",
    forecast: [
      { day: "월", temp: "13°", condition: "☁️" },
      { day: "화", temp: "12°", condition: "🌧️" },
      { day: "수", temp: "14°", condition: "🌦️" },
      { day: "목", temp: "15°", condition: "☁️" },
      { day: "금", temp: "14°", condition: "🌤️" },
      { day: "토", temp: "16°", condition: "☀️" },
      { day: "일", temp: "15°", condition: "☁️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "at",
    name: "오스트리아",
    nameEn: "Austria",
    emoji: "🇦🇹",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "비엔나 기준 맑고 쾌적. 기온 10-20도.",
    forecast: [
      { day: "월", temp: "18°", condition: "☀️" },
      { day: "화", temp: "19°", condition: "☀️" },
      { day: "수", temp: "17°", condition: "🌤️" },
      { day: "목", temp: "15°", condition: "☁️" },
      { day: "금", temp: "16°", condition: "🌦️" },
      { day: "토", temp: "19°", condition: "☀️" },
      { day: "일", temp: "20°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "se",
    name: "스웨덴",
    nameEn: "Sweden",
    emoji: "🇸🇪",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "스톡홀름 기준 쌀쌀하고 맑음. 기온 5-14도.",
    forecast: [
      { day: "월", temp: "10°", condition: "☀️" },
      { day: "화", temp: "11°", condition: "🌤️" },
      { day: "수", temp: "9°", condition: "☁️" },
      { day: "목", temp: "8°", condition: "🌧️" },
      { day: "금", temp: "12°", condition: "🌤️" },
      { day: "토", temp: "13°", condition: "☀️" },
      { day: "일", temp: "14°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "kr", code: "SEK", name: "크로나", nameEn: "Krona" }
  },
  {
    id: "cz",
    name: "체코",
    nameEn: "Czech Republic",
    emoji: "🇨🇿",
    voltage: "230V",
    plugType: "Type E",
    weatherSummary: "프라하 기준 흐리고 가끔 해. 기온 9-18도.",
    forecast: [
      { day: "월", temp: "16°", condition: "🌤️" },
      { day: "화", temp: "17°", condition: "☀️" },
      { day: "수", temp: "15°", condition: "☁️" },
      { day: "목", temp: "13°", condition: "🌧️" },
      { day: "금", temp: "14°", condition: "☁️" },
      { day: "토", temp: "17°", condition: "☀️" },
      { day: "일", temp: "18°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "Kč", code: "CZK", name: "코루나", nameEn: "Koruna" }
  },
  {
    id: "pt",
    name: "포르투갈",
    nameEn: "Portugal",
    emoji: "🇵🇹",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "리스본 기준 화창하고 온화함. 기온 16-24도.",
    forecast: [
      { day: "월", temp: "22°", condition: "☀️" },
      { day: "화", temp: "23°", condition: "☀️" },
      { day: "수", temp: "24°", condition: "☀️" },
      { day: "목", temp: "21°", condition: "🌤️" },
      { day: "금", temp: "20°", condition: "☁️" },
      { day: "토", temp: "22°", condition: "☀️" },
      { day: "일", temp: "24°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "nz",
    name: "뉴질랜드",
    nameEn: "New Zealand",
    emoji: "🇳🇿",
    voltage: "230V",
    plugType: "Type I",
    weatherSummary: "오클랜드 기준 쾌적함. 기온 14-20도 예상. 자연 활동에 적합합니다.",
    forecast: [
      { day: "월", temp: "18°", condition: "☀️" },
      { day: "화", temp: "19°", condition: "☀️" },
      { day: "수", temp: "17°", condition: "⛅" },
      { day: "목", temp: "16°", condition: "🌧️" },
      { day: "금", temp: "15°", condition: "☁️" },
      { day: "토", temp: "18°", condition: "☀️" },
      { day: "일", temp: "19°", condition: "☀️" },
    ],
    region: 'Oceania',
    currency: { symbol: "$", code: "NZD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "tr",
    name: "터키",
    nameEn: "Turkey",
    emoji: "🇹🇷",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "이스탄불 기준 대체로 맑음. 기온 15-23도. 일교차가 큽니다.",
    forecast: [
      { day: "월", temp: "20°", condition: "☀️" },
      { day: "화", temp: "22°", condition: "☀️" },
      { day: "수", temp: "23°", condition: "☀️" },
      { day: "목", temp: "21°", condition: "⛅" },
      { day: "금", temp: "19°", condition: "☁️" },
      { day: "토", temp: "21°", condition: "☀️" },
      { day: "일", temp: "22°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "₺", code: "TRY", name: "리라", nameEn: "Lira" }
  },
  {
    id: "gr",
    name: "그리스",
    nameEn: "Greece",
    emoji: "🇬🇷",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "아테네 기준 화창함. 기온 18-26도. 햇살이 강하니 주의하세요.",
    forecast: [
      { day: "월", temp: "22°", condition: "☀️" },
      { day: "화", temp: "24°", condition: "☀️" },
      { day: "수", temp: "25°", condition: "☀️" },
      { day: "목", temp: "26°", condition: "☀️" },
      { day: "금", temp: "23°", condition: "⛅" },
      { day: "토", temp: "25°", condition: "☀️" },
      { day: "일", temp: "26°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "fi",
    name: "핀란드",
    nameEn: "Finland",
    emoji: "🇫🇮",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "헬싱키 기준 쌀쌀함. 기온 5-12도. 방한 대책이 필요합니다.",
    forecast: [
      { day: "월", temp: "8°", condition: "🌤️" },
      { day: "화", temp: "9°", condition: "☀️" },
      { day: "수", temp: "7°", condition: "☁️" },
      { day: "목", temp: "6°", condition: "🌧️" },
      { day: "금", temp: "9°", condition: "🌤️" },
      { day: "토", temp: "11°", condition: "☀️" },
      { day: "일", temp: "12°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "gu",
    name: "괌",
    nameEn: "Guam",
    emoji: "🇬🇺",
    voltage: "120V",
    plugType: "Type A / B",
    weatherSummary: "연중 온화하고 습함. 기온 26-31도. 소나기 주의.",
    forecast: [
      { day: "월", temp: "29°", condition: "☀️" },
      { day: "화", temp: "30°", condition: "🌤️" },
      { day: "수", temp: "31°", condition: "🌦️" },
      { day: "목", temp: "30°", condition: "☀️" },
      { day: "금", temp: "29°", condition: "🌦️" },
      { day: "토", temp: "30°", condition: "☀️" },
      { day: "일", temp: "31°", condition: "☀️" },
    ],
    region: 'North America',
    currency: { symbol: "$", code: "USD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "mn",
    name: "몽골",
    nameEn: "Mongolia",
    emoji: "🇲🇳",
    voltage: "220V",
    plugType: "Type C / E",
    weatherSummary: "울란바토르 기준 맑고 건조함. 기온 5-15도. 밤에는 매우 춥습니다.",
    forecast: [
      { day: "월", temp: "10°", condition: "☀️" },
      { day: "화", temp: "12°", condition: "☀️" },
      { day: "수", temp: "14°", condition: "🌤️" },
      { day: "목", temp: "13°", condition: "☁️" },
      { day: "금", temp: "9°", condition: "☀️" },
      { day: "토", temp: "12°", condition: "☀️" },
      { day: "일", temp: "15°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "₮", code: "MNT", name: "투그릭", nameEn: "Tugrik" }
  },
  {
    id: "ie",
    name: "아일랜드",
    nameEn: "Ireland",
    emoji: "🇮🇪",
    voltage: "230V",
    plugType: "Type G (3-pin)",
    weatherSummary: "더블린 기준 흐리고 가랑비. 기온 10-15도. 우비 구비 권장.",
    forecast: [
      { day: "월", temp: "13°", condition: "🌧️" },
      { day: "화", temp: "12°", condition: "🌧️" },
      { day: "수", temp: "14°", condition: "☁️" },
      { day: "목", temp: "15°", condition: "⛅" },
      { day: "금", temp: "13°", condition: "🌧️" },
      { day: "토", temp: "14°", condition: "☁️" },
      { day: "일", temp: "15°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "€", code: "EUR", name: "유로", nameEn: "Euro" }
  },
  {
    id: "no",
    name: "노르웨이",
    nameEn: "Norway",
    emoji: "🇳🇴",
    voltage: "230V",
    plugType: "Type C / F",
    weatherSummary: "오슬로 기준 춥고 맑음. 기온 2-10도. 단단히 무장하세요.",
    forecast: [
      { day: "월", temp: "6°", condition: "☀️" },
      { day: "화", temp: "7°", condition: "☀️" },
      { day: "수", temp: "5°", condition: "☁️" },
      { day: "목", temp: "4°", condition: "🌧️" },
      { day: "금", temp: "8°", condition: "🌤️" },
      { day: "토", temp: "9°", condition: "☀️" },
      { day: "일", temp: "10°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "kr", code: "NOK", name: "크로네", nameEn: "Krone" }
  },
  {
    id: "dk",
    name: "덴마크",
    nameEn: "Denmark",
    emoji: "🇩🇰",
    voltage: "230V",
    plugType: "Type C / K",
    weatherSummary: "코펜하겐 기준 바람이 불고 흐림. 기온 8-14도.",
    forecast: [
      { day: "월", temp: "11°", condition: "☁️" },
      { day: "화", temp: "12°", condition: "🌧️" },
      { day: "수", temp: "13°", condition: "⛅" },
      { day: "목", temp: "12°", condition: "☁️" },
      { day: "금", temp: "11°", condition: "🌤️" },
      { day: "토", temp: "13°", condition: "☀️" },
      { day: "일", temp: "14°", condition: "☀️" },
    ],
    region: 'Europe',
    currency: { symbol: "kr", code: "DKK", name: "크로네", nameEn: "Krone" }
  },
  {
    id: "mp",
    name: "사이판",
    nameEn: "Saipan",
    emoji: "🇲🇵",
    voltage: "120V",
    plugType: "Type A / B",
    weatherSummary: "열대 기후, 맑고 습함. 기온 26-30도 예상.",
    forecast: [
      { day: "월", temp: "28°", condition: "☀️" },
      { day: "화", temp: "29°", condition: "☀️" },
      { day: "수", temp: "30°", condition: "🌤️" },
      { day: "목", temp: "29°", condition: "🌦️" },
      { day: "금", temp: "28°", condition: "☀️" },
      { day: "토", temp: "29°", condition: "☀️" },
      { day: "일", temp: "30°", condition: "☀️" },
    ],
    region: 'North America',
    currency: { symbol: "$", code: "USD", name: "달러", nameEn: "Dollar" }
  },
  {
    id: "mo",
    name: "마카오",
    nameEn: "Macao",
    emoji: "🇲🇴",
    voltage: "230V",
    plugType: "Type G / D",
    weatherSummary: "흐리고 다소 습함. 기온 23-27도 수준.",
    forecast: [
      { day: "월", temp: "25°", condition: "☁️" },
      { day: "화", temp: "26°", condition: "🌦️" },
      { day: "수", temp: "27°", condition: "☀️" },
      { day: "목", temp: "26°", condition: "⛅" },
      { day: "금", temp: "25°", condition: "☁️" },
      { day: "토", temp: "26°", condition: "☀️" },
      { day: "일", temp: "27°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "P", code: "MOP", name: "파타카", nameEn: "Pataca" }
  },
  {
    id: "in",
    name: "인도",
    nameEn: "India",
    emoji: "🇮🇳",
    voltage: "230V",
    plugType: "Type C / D / M",
    weatherSummary: "델리 기준 맑고 무더움. 기온 25-38도. 수분 섭취에 유의하세요.",
    forecast: [
      { day: "월", temp: "34°", condition: "☀️" },
      { day: "화", temp: "36°", condition: "☀️" },
      { day: "수", temp: "38°", condition: "☀️" },
      { day: "목", temp: "35°", condition: "🌤️" },
      { day: "금", temp: "33°", condition: "🌦️" },
      { day: "토", temp: "35°", condition: "☀️" },
      { day: "일", temp: "37°", condition: "☀️" },
    ],
    region: 'Asia',
    currency: { symbol: "₹", code: "INR", name: "루피", nameEn: "Rupee" }
  }
];

export const INITIAL_CHECKLIST: ChecklistItem[] = [
  // 필수 (Essential)
  { id: '1', name: '여권 (신분증)', nameEn: 'Passport (ID)', category: '필수', completed: false, icon: '🛂' },
  { id: '2', name: '여행자보험', nameEn: 'Travel Insurance', category: '필수', completed: false, icon: '📄' },
  { id: '3', name: '트래블카드 / 신용카드', nameEn: 'Travel Card / Credit Card', category: '필수', completed: false, icon: '💳' },
  { id: '4', name: '환전 (현지 화폐)', nameEn: 'Currency Exchange', category: '필수', completed: false, icon: '💵' },
  { id: '5', name: '마스크', nameEn: 'Face Mask', category: '필수', completed: false, icon: '😷' },
  
  // 전자기기 (Electronics)
  { id: '6', name: '변환콘센트 (멀티어댑터)', nameEn: 'Travel Adapter', category: '전자기기', completed: false, icon: '🔌' },
  { id: '7', name: '노트북 & 거치대', nameEn: 'Laptop & Stand', category: '전자기기', completed: false, icon: '💻' },
  { id: '8', name: '핸드폰 / 워치 충전기', nameEn: 'Phone / Watch Charger', category: '전자기기', completed: false, icon: '🔋' },
  { id: '9', name: '멀티탭 (긴 것)', nameEn: 'Power Strip', category: '전자기기', completed: false, icon: '🔌' },
  { id: '10', name: '보조배터리 (기내휴대)', nameEn: 'Power Bank', category: '전자기기', completed: false, icon: '🔋' },
  { id: '11', name: '이어폰 / 비행기 귀마개', nameEn: 'Earphones / Earplugs', category: '전자기기', completed: false, icon: '🎧' },
  
  // 세면/위생 (Toiletries)
  { id: '12', name: '슬리퍼 (기내/호텔용)', nameEn: 'Slippers', category: '세면/위생', completed: false, icon: '🩴' },
  { id: '13', name: '면도기 / 칫솔 / 치약', nameEn: 'Razor / Toothbrush / Paste', category: '세면/위생', completed: false, icon: '🪥' },
  { id: '14', name: '치실 / 워터픽', nameEn: 'Floss / Waterpick', category: '세면/위생', completed: false, icon: '🦷' },
  { id: '15', name: '바디로션 / 얼굴로션', nameEn: 'Body / Face Lotion', category: '세면/위생', completed: false, icon: '🧴' },
  { id: '16', name: '물티슈 / 휴지', nameEn: 'Wet Wipes / Tissues', category: '세면/위생', completed: false, icon: '🧻' },
  { id: '17', name: '안경닦이 / 빗', nameEn: 'Glass Cleaner / Comb', category: '세면/위생', completed: false, icon: '👓' },
  { id: '18', name: '우산 / 모자 / 핫팩', nameEn: 'Umbrella / Hat / Heat Pack', category: '세면/위생', completed: false, icon: '🌂' },
  
  // 의류 (Clothes)
  { id: '19', name: '정장 (자켓/바지/셔츠)', nameEn: 'Suit (Jacket/Pants/Shirt)', category: '의류', completed: false, icon: '👔' },
  { id: '20', name: '코트 / 스웨터', nameEn: 'Coat / Sweater', category: '의류', completed: false, icon: '🧥' },
  { id: '21', name: '얇은패딩 / 바람막이', nameEn: 'Down Jacket / Windbreaker', category: '의류', completed: false, icon: '🧥' },
  { id: '22', name: '내복 / 잠옷 / 실내복', nameEn: 'Underwear / Pajamas', category: '의류', completed: false, icon: '👕' },
  { id: '23', name: '속옷 / 양말 / 손수건', nameEn: 'Underwear / Socks / Handkerchief', category: '의류', completed: false, icon: '🧦' },
  { id: '24', name: '장갑', nameEn: 'Gloves', category: '의류', completed: false, icon: '🧤' },
  
  // 식량/상비약 (Food/Medicine)
  { id: '25', name: '영양제 / 상비약 (타이레놀 등)', nameEn: 'Vitamins / Medicine', category: '식량/상비약', completed: false, icon: '💊' },
  { id: '26', name: '햇반 / 컵라면 / 김치', nameEn: 'Instant Rice / Ramen / Kimchi', category: '식량/상비약', completed: false, icon: '🍜' },
  { id: '27', name: '말린 국 / 김 / 견과류', nameEn: 'Dried Soup / Seaweed / Nuts', category: '식량/상비약', completed: false, icon: '🥜' },
  { id: '28', name: '숟가락 / 나무젓가락 / 과일칼', nameEn: 'Spoon / Chopsticks / Knife', category: '식량/상비약', completed: false, icon: '🍴' },
  { id: '29', name: '종이컵 / 위생봉투', nameEn: 'Paper Cups / Plastic Bags', category: '식량/상비약', completed: false, icon: '🥤' },
  
  // 기타 (Others)
  { id: '30', name: '세탁물 봉지 / 키친타올', nameEn: 'Laundry Bags / Kitchen Towel', category: '기타', completed: false, icon: '🧺' },
  { id: '31', name: '에코백 / 여유 가방', nameEn: 'Eco Bag / Extra Bag', category: '기타', completed: false, icon: '👜' },
  { id: '32', name: '캐리어 내부 사진 찍기', nameEn: 'Take luggage photos', category: '기타', completed: false, icon: '📸' },
];
